...
sub main {
    ...
    my $transactions_file = $ARGV[0] || 't/Settlement/data/transactions.csv';

    # output files (argv[2] and argv[3] or use defaults)
    my $t_plus_1_file = $ARGV[1] || 't/Settlement/data/t1.csv';
    my $t_plus_2_file = $ARGV[2] || 't/Settlement/data/t2.csv';
    ...
    Settlement::Settlement::process_lines(
        Settlement::Reader::read_as_aoa($transactions_file),
        \@t_plus_1, \@t_plus_2 );

    # write out t_plus_1, t_plus_2 transactions
    printf "Writing T+1 to: %s\n", $t_plus_1_file;
    Settlement::Writer::write_aoa( $t_plus_1_file, \@t_plus_1 );
    printf "Writing T+2 to: %s\n", $t_plus_2_file;
    Settlement::Writer::write_aoa( $t_plus_2_file, \@t_plus_2 );
